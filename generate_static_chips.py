...
def build_static_chips(stock_list, output_file, trend_days=None, concentration_threshold=None, sleep_sec=0.2):
    ...
    try:
        ...
        print(
            f"Cannot check FinMind usage, continue chip build: {e}", flush=True)

    rows = []
    for i, s in enumerate(stock_list, 1):
        sid = str(s.get("stock_id", "")).strip()
        print(
            f"Processing chips {i}/{len(stock_list)}: {sid} {s.get('name')}", flush=True)
        rows.append(build_chip_row(s, trend_days=trend_days,
                    concentration_threshold=concentration_threshold))
    ...
    print(
        f"Static_Chips rebuild: {status_counts}, total={len(final_df)}", flush=True)


def main():
    parser = argparse.ArgumentParser(
        description="Rebuild Static_Chips.csv for broker chip data.")
    parser.add_argument("--output", default=os.getenv("STATIC_CHIPS_FILE",
                        getattr(config, "STATIC_CHIPS_OUTPUT_FILE", "Static_Chips.csv")))
