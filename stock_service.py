...
def process_stock(s, static_map=None):
    ...
    try:
        ...
        try:
            ...
            ) or {"signal": "", "reason": "", "signal_text": "技術面：資料不足\n籌碼面：資料不足"}
        except Exception as e:
            print(f"❌ signal error {stock_id}: {e}")
            signal_res = {"signal": "",
                          "reason": f"signal error: {e}", "signal_text": "技術面：資料不足\n籌碼面：資料不足"}

        signal = signal_res.get("signal", "")
        reason = signal_res.get("reason", "")
        signal_text = signal_res.get("signal_text", "技術面：資料不足\n籌碼面：資料不足")
        ...
        sig = 1 if signal == "轉強" else -1 if signal == "調節" else 0
        ...
        if "短線過熱" in reason or "不宜追價" in reason:
            entry_note = "不追價"
        elif signal == "轉強" and kd_buy and ma18_break and k is not None and k < 35:
            entry_note = "抄底"
        elif signal == "轉強" and ma18_break and chgPct >= 3:
            entry_note = "追漲"
